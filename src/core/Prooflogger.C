    ...
    */
    ...
    if(sign(definition[2]) == 1 && sigma > 0) {
        ...
        if(second > formula_length + n_variables && constraint_store.find(second) != constraint_store.end()) {
            int third_constraint_id = resolved_one ? -tree_constraint_counter : constraint_counter;
            ...
            tree_derivation.push(new Operation(new Operation(new Single(third_constraint_id ), new Single(second_coeff), "*"),
    ...
    if(resolved_one) return -tree_constraint_counter;
